export default function createModal(){
    ...
    <style>
      .modal-container {
        position: fixed;
        inset: 0;
        background-color: hsl(206 70% 3% / .7);
        display: grid;
        place-items: center;
        opacity: 0;
        pointer-events: none;
      ...
      .modal {
        ...
        place-items: center;
        overflow: hidden;
        box-shadow: 0px 2px 40px hsl(206 70% 0%);
      }
      ...
      .modal__btn {
        display: grid;
        place-items: center;
        background-color: hsl(206 50% 96% / .5);
        color: hsl(206 70% 9%);
        padding: .5rem;
      ...
      .modal__btn:is(:hover,:focus) {
        background-color: hsl(206 50% 96% / .7);
        border: 4px solid hsl(206 50% 96% / .8);
      }
      ...
      .modal__indicator {
        ...
        height: 1rem;
        border-radius: 50%;
        background-color: hsl(206 50% 96% / .4);
        border: 3px solid hsl(206 50% 96% / .6);
        cursor: pointer;
        position: relative;
      }
      ...
      .modal__indicator:is(:hover, :focus) {
        background-color: hsl(206 50% 96% / .5);
        border: 3px solid hsl(206 50% 96% / .7);
      }
      ...
      .modal__indicator.active::before {
        ...
        inset: 2px;
        border-radius: 50%;
        background-color: hsl(206 50% 96% / 0.9);
      }
    </style>
    `)
  }